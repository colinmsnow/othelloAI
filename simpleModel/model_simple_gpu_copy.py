...
class DQN(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = x.view(-1, 4096)
        # x = F.relu(self.fc1(x))
        # x = F.dropout(F.relu(self.fc2(x)), p = 0.3, training = True)
        # x = F.dropout(F.relu(self.fc3(x)), p = 0.3, training = True)

        x = F.dropout(F.relu(self.bn1(self.conv1(x))), p = 0.3, training=True)
        x = x.view(-1, 1296)
